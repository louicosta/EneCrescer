...
const updateSeriesById = async (req, res) => {
  try {
    ...
    const findSerie = await Movie.findById(id)
    if (findSerie == null) {
    ...
    findSerie.originalBroadcaster = originalBroadcaster || findSerie.originalBroadcaster
...
const deleteSeriesById = async (req, res) => {
    ...
    await jwt.verify(token, SECRET, async function (err) {
      ...
      res.status(200).json({ message: `The serie ${findSerie.title} was successfully deleted` });
